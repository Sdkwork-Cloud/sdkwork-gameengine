...
profiles use structured `SDKWORK_DATABASE_*` fields. `SDKWORK_DATABASE_URL` remains a
private explicit override for protected operator-managed environments only; checked-in production
topology profiles use `SDKWORK_DATABASE_ENGINE`, host, port, name, schema, username,
`SDKWORK_DATABASE_PASSWORD_FILE`, SSL mode, max connections, and
`SDKWORK_DATABASE_AUTO_MIGRATE=false`.

Production startup fails closed when neither structured `SDKWORK_DATABASE_ENGINE` nor the
private explicit URL override is present. Structured PostgreSQL production config also fails closed
when `SDKWORK_DATABASE_PASSWORD_FILE` is missing. Development topology profiles may use a
local inline `SDKWORK_DATABASE_PASSWORD` for local PostgreSQL bootstrap only.
...
13. Use structured `SDKWORK_DATABASE_*` runtime config for server/container profiles.
