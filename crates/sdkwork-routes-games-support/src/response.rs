...
use crate::correlation::GamesProblemCorrelation;
use crate::problem::GamesApiProblem;

pub fn resolved_trace_id() -> String {
    if let Some(correlation) = GamesProblemCorrelation::current() {
...
mod tests {
    ...
    async fn success_handler() -> axum::response::Response {
        success_resource_response(serde_json::json!({ "gameCode": "xiangqi" }))
    }
    ...
    async fn success_response_uses_sdkwork_api_response_envelope() {
        ...
        assert_eq!(
            "xiangqi",
            payload["data"]["item"]["gameCode"].as_str().unwrap()
        );
    }
}
